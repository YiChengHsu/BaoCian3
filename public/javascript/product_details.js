...
//Connect to socket with token if exist
const socket = io({
	auth: {
...
// Get the real-time room user counts
...
fetch(detailsUrl, {
	...
	.then((res) => {
		...
		$("#bread-subcategory").attr("href", `/product/${data.sub_category}`).text(category.subCategory)
		...
		} else if (data.highest_user_id != undefined && data.highest_user_id == userId) {
...
form.addEventListener("submit", (e) => {
	...
	const bidAmount = Number(currentAmount.replace("$", "").replace(",", "")) + userBidIncr
	const totalBidTimes = bidTimes

	Swal.fire({
		title: "確認出價",
		html: `<p>目前出價：<b>${currentAmount}</b></p><p>你的出價：<b>$${toCurrency(bidAmount)}</b></p>`,
		icon: "warning",
		showCancelButton: true,
...
socket.on(`updateProduct${productId}`, (bid) => {
...
$("#report-button").click(async () => {
	...
	const { value: reportObj } = await Swal.fire({
		title: "檢舉這個商品",
		html: `<select name="cause" class='swal2-select' id='cause'>
...
const renderBidRecord = (record) => {
	...
	recordDiv.textContent = `$${toCurrency(record.bid_amount)}`
...
const toCurrency = (num) => {
	const parts = num.toString().split(".") || num.toString()
