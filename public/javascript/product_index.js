const params =
	window.location.pathname == "/" ? "/product/all" : window.location.pathname
...
socket.on("roomUsers", (data) => {
	roomUsersCounts = data
	Object.keys(roomUsersCounts).map((e) => {
		$(`#room-user-${e}`).text(data[e] || 0)
	})
})
...
fetch("/api/1.0" + params + query, {
	...
	.then((res) => {
		...
		if (params != "/product/all" && data.length == 0) {
			Swal.fire({
		...
		data.map((e) => {
			...
			const sellerRating = e.sellerInfo.rating
				? e.sellerInfo.rating.toFixed(2)
				: "尚未評分"
      ...
      <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/></svg><span id='seller-rating-${id}'>${sellerRating} </span>`
			...
			productImage.className =
				"card-img rounded-2 product-image w-100 text-end p-2 pt-3"
			productImage.style.backgroundImage = `url(${e.main_image})`


			if (e.highest_user_id && e.highest_user_id == userId) {
				$("<span>", {
					id: `highest-tag-${id}`,
					text: '得標中',
			...
			if (e.auction_end == 2) {
				$("<span>")
					.addClass("p-2 btn-primary disabled rounded rounded-pill")
					.text("審核中")
					.appendTo(productImage)
			} else if (
				e.seller_id == userId &&
				e.end_time < Date.now() &&
				e.highest_user_id == null
			) {
				$("<span>")
					.addClass("p-2 btn-secondary disabled rounded rounded-pill")
					.text("沒人要QQ")
					.appendTo(productImage)
			} else if (e.seller_id == userId && e.end_time < Date.now()) {
				$("<span>")
					.addClass("p-2 btn-info disabled rounded rounded-pill")
					.text("賣出了")
					.appendTo(productImage)
			} else if (e.seller_id == userId) {
				$("<span>")
					.addClass("p-2 btn-success disabled rounded rounded-pill")
					.text("自己的")
					.appendTo(productImage)
			}
			...
			const roomUserNum = roomUsersCounts[id] || 0
      ...
      <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z"/>
			...
			watchBtn.addEventListener("click", (e) => {
				...
				fetch("/api/1.0/product/watchList", {
					method: "post",
					headers: {
			...
			socket.on(`updateProduct${id}`, (bid) => {
				...
				const highestBid = document.querySelector(`#highest-bid-${id}`)
				highestBid.textContent = `$${toCurrency(bid.bid_amount)}`

				const bidTimesDiv = document.querySelector(`#bid-times-${id}`)
				bidTimesDiv.textContent = bid.total_bid_times
		...
		for (let i = 0; i < totalPage; i++) {
			if (i === currentPage) {
				...
				$(
					`<li class="page-item"><a class="page-link" href="?paging=${i}">${
						i + 1
					}</a></li>`
				).insertBefore(".next-page")
			}
