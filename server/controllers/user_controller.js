...
const pageSize = 5;
const {getProductSellerInfo, getProductsImages, getProductWatchTimes} = require('../controllers/product_controller');
...
const getUserList = async (req, res) => {
  ...
  const findUserProduct = async (records) => {
    ...
    switch (records) {
      case 'like': productList = watchList
        return await Product.getProducts(pageSize, paging, {productList, order})
      case 'bade': productList = await Bid.getUserBadeProducts(userId)
